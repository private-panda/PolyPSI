...
/*for the sender setup*******************************************************************/
    ...
    
    {
            ...
            for(int i=0; i<nx; i++){
                ...
                for(int j=0; j<2; j++){
                    ...
                    for(int k=0; k<hash_num; k++){
                        ...
                        // if(rear >= B){
                        //     std::cout<<"the bin "<<i<<", "<<j<<" is full"<<std::endl;
                        //     if(j == 0){
                        //         zero_count++;
                        //     }
                        //     if(j == 1){
                        ...
                        // }
                        // if(i%(1<<10)==0)
                        // std::cout<<"("<<g_hash_1d_index<<", "<<g_hash_2d_index<<", "<<rear<<") \t";
        ...
        //insert random dummies for each sub-bin
            ...
            for(int i=0; i<omega; i++){
                for(int j=0; j<nu; j++){
                    ...
                    while(rear < B){
                        rnd = distrib(gen);
                        x_simple_hashing[i][j][rear].r = rnd%plain_modulus; rnd /= plain_modulus;
                        x_simple_hashing[i][j][rear].m = rnd%plain_modulus; rnd /= plain_modulus;
                        x_simple_hashing[i][j][rear].l = rnd;
                        rear++;
                    }
                }
            }
        ...
        //now we do interpolation for each sub-sub-bins
    ...
    for(int i=0; i<window_size; i++){
        ...
        // std::cout<<"receive ctxt y^"<<i+1<<std::endl;
    }
    ...
    for(int j=0; j<nu; j++){
        for(int k=0; k<alpha; k++){
            ...
            //randomize
            evaluator.multiply_plain_inplace(ctxt_P_l, ptxt_rnd);
            evaluator.relinearize_inplace(ctxt_P_l, relin_keys);
            // evaluator.multiply_plain_inplace(ctxt_P_m, ptxt_rnd);

            // do mudulus switching to save communication cost
            ...
            while(data_context->next_context_data()){
                ...
                // evaluator.mod_switch_to_next_inplace(ctxt_P_m);
...
std::vector<std::array<uint64_t, 2>> poly_low_receiver(std::vector<std::array<uint64_t, 2>>& Y, const poly_parms &p_parms){
    ...
    if(nx == (1<<16)){
        ...
        if(ny == (1<<10)){
            ...
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {32, 32, 32, 32, 32, 32, 32})); 
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {26, 26, 26, 26, 27, 27})); 
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {32, 33, 33, 33, 33})); 
            parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {32, 32, 32, 32, 32})); 
        ...
        if(ny == 5535){
            ...
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {32, 33, 33, 33, 33, 33})); 
            // std::cout<<"poly modulus: "<<poly_modulus_degree<<std::endl;
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {27, 27, 27, 27, 27, 27})); 
            parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {32, 32, 32, 32, 32})); 

            

            // parms.set_coeff_modulus(CoeffModulus::BFVDefault(poly_modulus_degree, seal::sec_level_type::tc192));
        ...
        if(ny == (1<<10)){
            ...
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {39, 39, 40, 40, 40}));
            parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {37, 37, 38, 38, 38}));

        }else if(ny == (1<<12)){
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {31, 31, 31, 31, 31, 32, 32, 32, 32, 32}));
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {39, 39, 39, 40, 40}));
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {27, 27, 27, 27, 27, 27, 27}));
            parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {32, 32, 32, 32, 32, 32}));
        ...
        if(ny == 5535){
            ...
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {40, 40, 41, 41, 41})); 

        }else if(ny == 11041){
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {36, 36, 36, 36, 36})); 
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {33, 33, 33, 34, 34, 34})); 
            parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {32, 32, 32, 32, 32, 32})); 
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {36, 36, 36, 36, 36, 36, 36})); 


            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {58, 58, 58, 59, 59, 59})); //for d=3
            // parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, {41, 41, 42, 42, 42})); 
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if(log_nx == 28){
        ...
        p_parms.modulus = 65537;
        p_parms.B = 192; //actually, it is B=191. 192 = 2^6 * 3
        // p_parms.alpha = 6;
        p_parms.alpha = 6;
        // p_parms.B = 200; //actually, it is B=191. 192 = 2^6 * 3
        // // p_parms.alpha = 6;
        // p_parms.alpha = 20;
        ...
        p_parms.modulus = 65537;
        // p_parms.B = 40; ////actually, it is B=37.
        // p_parms.alpha = 5;
        // p_parms.B = 42; ////actually, it is B=37.
        // p_parms.alpha = 6;
        p_parms.B = 40; ////actually, it is B=37.
        p_parms.alpha = 5;
        // alpha_min = 5;
