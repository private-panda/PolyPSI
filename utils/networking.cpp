...
void Networking::set_seal_context(shared_ptr<seal::SEALContext> new_context) {
...
void Networking::read_HEParms(seal::EncryptionParameters &parms){
    assert(read_uint32() == NET_MAGIC_PARMS);
    uint32_t length = read_uint32();
    auto transferred = read(socket, read_buffer, transfer_exactly(length));
    assert(transferred == length);
    parms.load(read_stream);
}
void Networking::write_HEParms(seal::EncryptionParameters &parms){
    write_uint32(NET_MAGIC_PARMS);
    parms.save(write_stream);
    uint32_t length = write_buffer.size();
    std::cout<<"before transfer parms length: "<<length<<std::endl;
    write_uint32(length);
    std::cout<<"before transfer parms: "<<length<<std::endl;
    auto transferred = write(socket, write_buffer, transfer_exactly(length));
    std::cout<<"after transfer parms: "<<length<<std::endl;
    
    assert(transferred == length);
    write_buffer.consume(length);
}

void Networking::read_ciphertext(seal::Ciphertext &ciphertext) {
    assert(seal_context);
...
void Networking::write_ciphertext(seal::Ciphertext &ciphertext) {
...
void Networking::read_ciphertexts(vector<seal::Ciphertext> &ciphertexts) {
...
void Networking::write_ciphertexts(vector<seal::Ciphertext> &ciphertexts) {
...
void Networking::read_secret_key(seal::SecretKey &secret_key) {
    assert(seal_context);
    assert(read_uint32() == NET_MAGIC_SECRET_KEY);
    uint32_t length = read_uint32();
    auto transferred = read(socket, read_buffer, transfer_exactly(length));
    assert(transferred == length);
    secret_key.load(seal_context, read_stream);
}

void Networking::write_secret_key(seal::SecretKey &secret_key) {
    write_uint32(NET_MAGIC_SECRET_KEY);
    secret_key.save(write_stream);
    uint32_t length = write_buffer.size();
    write_uint32(length);
    auto transferred = write(socket, write_buffer, transfer_exactly(length));
    assert(transferred == length);
...
void Networking::read_public_key(seal::PublicKey &public_key) {
    assert(seal_context);
...
void Networking::write_public_key(seal::PublicKey &public_key) {
...
void Networking::read_relin_keys(seal::RelinKeys &relin_keys) {
    assert(seal_context);
...
void Networking::write_relin_keys(seal::Serializable<seal::RelinKeys> &relin_keys) {
